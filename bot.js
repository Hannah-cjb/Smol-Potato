...
async function respond(key, authorId, userText, sink) {
  try {
    ...
    if (onCooldown(authorId, cooldownMs)) {
      if (!isSlow) {
        ...
        if (now - (lastMarinatedAt.get(authorId) || 0) > 10000) {
          ...
          await sink.send('Smol Potato is marinating — give me a second and try again!');
        }
      }
      return;
    }
    ...
    try {
      ...
      replyText = 'My spuds got tangled — give me a second and try again!';
    }
...
client.once(Events.ClientReady, async (c) => {
  ...
  const askCommand = new SlashCommandBuilder()
    .setName('ask')
    .setDescription('Chat with Smol Potato AI')
    .addStringOption((opt) =>
      opt
        .setName('message')
...
const server = http.createServer((req, res) => {
  ...
  res.writeHead(200, { 'Content-Type': 'text/plain' });
  res.end('Smol Potato AI is running.');
});
